...
function SpendingBreakdown() {  
  const { spendingsBreakdown, transactions, accounts, categories } = useLoaderData();  
  const [actionSpendingsBreakdown, setActionSpendingsBreakdown] = useState(null);  
  ...
  async function handleFiltersSubmit(event) {  
    ...
    async function fetchSpendingBreakdown() {  
      try {  
        const spendingBreakdownResponse = await apiService.getSpendingsByCategories({  
  ...
  const { minDate: calculatedMinDate, maxDate: calculatedMaxDate } = useMemo(() => {  
    if (!transactions || transactions.length === 0) {  
      return { minDate: null, maxDate: null };  
    }  
    const transactionDates = transactions.map((tx) => dayjs.utc(tx.transaction_date));  
    ...
    <div className={styles.spendingBreakdownMain}>  
      ...
      <div className={styles.spendingBreakdownContent}>  
        <div className={styles.spendingBreakdownContentLeftColumn}>  
          <div className={styles.spendingBreakdownContentChart}>  
            <div className={styles.spendingBreakdownContentChartHeader}>  
              <div className={styles.spendingBreakdownContentChartHeaderSpendingWrapper}>  
        ...
        <div className={styles.spendingBreakdownContentSidebar}>  
          ...
          <VerticalBoxPlot data={actionSpendingsBreakdown  
                    ? actionSpendingsBreakdown.spendingsByCategories  
                    : spendingsBreakdown.spendingsByCategories} />  
          </div>  
        </div>  
